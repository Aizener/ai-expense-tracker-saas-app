...
async function Layout({ children }: PropsWithChildren) {
  ...
  if (userId) {
    return (
      <div
        className="mt-8 mx-auto flex justify-center items-center px-30 py-16 text-lg font-bold bg-gradient-to-r from-blue-500 via-sky-500 to-teal-400 rounded-md shadow-xl text-white"
      >
        跳转中...
        {children}
      </div>
  ...
  return (
    <div className="flex justify-center max-auto my-4 lg:my-10 gap-x-8 w-full">
      <div className="hidden lg:block flex-1 bg-gradient-to-br from-blue-100 via-sky-100 to-teal-100 rounded-md p-10 shadow-xl">
        <h1 className='text-3xl sm:text-4xl md:text-3xl lg:text-3xl xl:text-5xl font-bold mb-4 sm:mb-6 text-gray-900 dark:text-gray-100 leading-tight'>
          加入{' '}
          <span className='bg-gradient-to-r from-blue-500 via-sky-500 to-teal-400 bg-clip-text text-transparent'>
            ExpenseTracker AI
          </span>
        </h1>
        <p className="text-sm text-secondary-foreground mt-4">您的智能消费分析工具，总结您的消费问题。</p>
        <p className="text-sm text-secondary-foreground mt-4">并提出建议，帮助您合理规划支出，避免不必要开销，优化理财习惯。</p>
